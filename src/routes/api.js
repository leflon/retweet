...
router.post("/tweets/add", upload.single("image"), async (req, res) => {
    ...
    if (repliesTo) {
        ...
        if (!parentTweet || parentTweet.isDeleted)
            return res
                .status(400)
                .send({ message: "Ce tweet répond à un tweet inexistant." });
        // On récupère le nom d'utilisateur de l'auteur du tweet auquel on répond ici.
        [[{ username: repliesToUsername }]] = await req.app.db.connection.query(
    ...
    if (req.file) {
        ...
        imageId = await req.app.db.addImage(
    ...
    const tweet = await req.app.db.addTweet({
        content,
    ...
    if (followers.length) {
        const [subs] = await req.app.db.connection.query(
            "SELECT subscription FROM Subscription WHERE user_id IN (?)",
            [followers],
        );
        const notification = {
            ...
            icon:
                process.env.APP_URL +
                `/public/${req.user.avatarId || "default_avatar"}.jpg`,
...
router.get("/tweets/like/:id", async (req, res) => {
    ...
    await tweet.save();
    const [subs] = await req.app.db.connection.query(
        "SELECT subscription FROM Subscription WHERE user_id = ?",
        [tweet.authorId],
    );
    if (subs.length) {
        const notification = {
...
router.get("/tweets/retweet/:id", async (req, res) => {
    ...
    if (tweet.retweets.includes(req.user.id))
        return res
            .status(400)
            .send({ message: "Vous avez déjà retweeté ce tweet." });
    ...
    let [subs] = await req.app.db.connection.query(
        "SELECT subscription FROM Subscription WHERE user_id = ?",
        [tweet.authorId],
    );
    if (subs.length) {
    ...
    if (req.user.followers.length) {
        ...
        [subs] = await req.app.db.connection.query(
            "SELECT subscription FROM Subscription WHERE user_id IN (?) AND user_id <> ?",
            [req.user.followers, tweet.authorId],
        );
        if (subs.length) {
...
router.get("/tweets/unretweet/:id", async (req, res) => {
    ...
    if (!tweet.retweets.includes(req.user.id))
        return res
            .status(400)
            .send({ message: "Vous n'avez pas retweeté ce tweet." });
    ...
    const [[{ id }]] = await req.app.db.connection.query(
        "SELECT id FROM Tweet WHERE content LIKE ? AND author_id = ? AND is_deleted = 0",
        [`%//RT:${req.params.id}%`, req.user.id],
    );
...
router.get("/tweets/delete/:id", async (req, res) => {
    ...
    if (tweet.authorId !== req.user.id && !req.user.isAdmin)
        return res
            .status(403)
            .send("Vous n'avez pas la permission de supprimer ce tweet.");
    ...
    if (tweet.content.match(/^\/\/RT:[\w-]{16}$/)) {
        const original = await req.app.db.getTweet(
            tweet.content.match(/^\/\/RT:[\w-]{16}$/)[0].slice(5),
        );
    ...
    if (tweet.retweets.length)
        req.app.db.connection.query(
            "UPDATE Tweet SET is_deleted = 1 WHERE content LIKE ?",
            [`%//RT:${tweet.id}%`],
        );
    await tweet.delete();
...
router.post(
    ...
    async (req, res) => {
        ...
        if (location.length > 30)
            return res
                .status(400)
                .send({ message: "Localisation trop longue." });
        if (website.length > 100)
            return res
                .status(400)
                .send({ message: "Site internet trop long." });
        if (req.files.avatar) {
            const avatarId = await req.app.db.addImage(
                join(__dirname, "../../", req.files.avatar[0].path),
                {
                    id: await req.app.db.generateId(),
                    type: "avatar",
                },
        ...
        if (req.files.banner) {
            const bannerId = await req.app.db.addImage(
                join(__dirname, "../../", req.files.banner[0].path),
                {
                    id: await req.app.db.generateId(),
                    type: "banner",
                },
...
router.get("/follow/:id", async (req, res) => {
    ...
    if (user.id === req.user.id)
        return res
            .status(400)
            .send({ message: "Vous ne pouvez pas vous suivre vous-même." });
    if (user.followers.includes(req.user.id))
        return res
            .status(400)
            .send({ message: "Vous suivez déjà cet utilisateur." });
...
router.get("/unfollow/:id", async (req, res) => {
    ...
    if (!user.followers.includes(req.user.id))
        return res
            .status(400)
            .send({ message: "Vous ne suivez pas cet utilisateur." });
...
router.post("/subscription", async (req, res) => {
    ...
    await req.app.db.connection.query(
